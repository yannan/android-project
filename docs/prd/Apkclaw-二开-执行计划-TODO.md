...
| T23 | 进行中 | 完成集成验收：绑定、接单、执行、历史、设置五块主流程全量回归 | 验收记录与问题清单 | T16, T18, T20, T21, T22 | 2026-05-03 | Cursor | **T19 暂缓**；模板单列「不适用」。已新增 `docs/prd/Apkclaw-T23-集成验收清单.md`；研发冒烟 `./gradlew :app:compileDebugKotlin`（ApkClaw）通过。**待办**：按清单真机逐项勾选并回填本文「问题清单」后可将状态改为已完成 |
